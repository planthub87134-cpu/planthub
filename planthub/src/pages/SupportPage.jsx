    ...
    <div className="page-container" style={{ padding: 'var(--space-6) 0' }}>
      <div style={{ maxWidth: '1000px', margin: '0 auto', padding: '0 var(--space-4)' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 2fr', gap: 'var(--space-6)' }}>
          ...
          <div style={{ backgroundColor: 'white', borderRadius: 'var(--radius-lg)', border: '1px solid var(--border-light)', display: 'flex', flexDirection: 'column', height: '600px' }}>
            {activeTicket === 'new' ? (
              <div style={{ padding: 'var(--space-6)' }}>
                <h2 style={{ marginBottom: 'var(--space-4)', fontSize: 'var(--text-xl)' }}>Create a New Ticket</h2>
                <form onSubmit={handleCreateTicket}>
                  <div style={{ marginBottom: 'var(--space-4)' }}>
                    <label style={{ display: 'block', marginBottom: 'var(--space-2)', fontWeight: 'var(--font-medium)' }}>Subject</label>
                    <input 
                      type="text"
                      className="input"
                      value={newSubject}
                  ...
                  <div style={{ marginBottom: 'var(--space-4)' }}>
                    <label style={{ display: 'block', marginBottom: 'var(--space-2)', fontWeight: 'var(--font-medium)' }}>Message</label>
                    <textarea 
                      className="input"
              ...
              <>
                <div style={{ padding: 'var(--space-4)', borderBottom: '1px solid var(--border-light)', backgroundColor: 'var(--bg-subtle)' }}>
                  ...
                  <div style={{ fontSize: 'var(--text-sm)', color: 'var(--text-muted)' }}>Ticket ID: {currentTicket.id}</div>
                </div>
                    ...
                    return (
                      <div key={msg.id} style={{ 
                        ...
                        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 'var(--space-2)', fontSize: 'var(--text-xs)', color: 'var(--text-muted)' }}>
                          <span style={{ fontWeight: 'var(--font-bold)', color: isCustomer ? 'var(--primary-700)' : 'var(--text-secondary)' }}>{msg.senderName}</span>
                ...
                {currentTicket.status === 'Open' ? (
                  ...
                  <div style={{ padding: 'var(--space-4)', borderTop: '1px solid var(--border-light)', textAlign: 'center', color: 'var(--text-muted)' }}>
                    This ticket is resolved and closed for replies.
                  </div>
                )}
              </>
            ) : (
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '100%', color: 'var(--text-muted)' }}>
                Select a ticket from the left or create a new one.
              </div>
            )}
          </div>
        </div>
